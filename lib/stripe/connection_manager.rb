...
module Stripe
  ...
  class ConnectionManager
    ...
    # `u` should be a parsed `URI` object.
    private def create_connection(u)
      # These all come back as `nil` if no proxy is configured.
      proxy_host, proxy_port, proxy_user, proxy_pass = get_proxy_parts

      connection = Net::HTTP.new(u.host, u.port,
        proxy_host, proxy_port, proxy_user, proxy_pass)

      connection.open_timeout = Stripe.open_timeout
      connection.read_timeout = Stripe.read_timeout

      connection.use_ssl = u.scheme == "https"
