...
module Stripe
  ...
  class StripeClient
    ...
    def initialize(connection_manager = nil)
      self.connection_manager = connection_manager || self.class.default_connection_manager
    ...
    def execute_request(method, path,
      ...
      body = body_params ? Util.encode_parameters(body_params) : nil
    ...
    NETWORK_ERROR_MESSAGES_MAP = {
      ...
      [
        ...
        "Could not establish a secure connection to Stripe, you " \
    ...
    private def execute_request_with_rescues(api_base, context)
      ...
      begin
        ...
        # TODO: maybe call `resp.value` instead which raises error
      ...
      # type of exception that we didn't expect to handle.
        ...
        when Stripe::StripeError
          raise
        when *NETWORK_ERROR_MESSAGES_MAP.keys.flatten
    ...
    private def handle_network_error(error, context, num_retries,
      ...
      errors, message = NETWORK_ERROR_MESSAGES_MAP.detect do |errors, _message|
        errors.include?(error)
      end

      if errors.nil?
        message = "Unexpected error #{error} communicating with Stripe. " \
          "Please let us know at support@stripe.com."
      end

      # Not all messages want this parameter (only those with `%s` get it), but
      # it's not destructive to run `%` even on those that don't.
      api_base ||= Stripe.api_base
